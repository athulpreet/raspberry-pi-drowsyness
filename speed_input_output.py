...
==============================================================================
 ...
 v9 -- MUTED means muted: below 5 km/h detection stops within 0.5 s
==============================================================================
 ...
 Safety rules (SlabSelector):
   ...
   MUTED         (a first slab with every level OFF) is special: raw speed
                 below its edge for mute_confirm_s (0.5 s) mutes at once, even
                 during an alarm. Detection PAUSES: no inference, no events,
                 no buzzer, no speed byte, LED off. Speed is still read, and
                 the first reading above the edge resumes detection.
...
class Config:
    ...
    mute_confirm_s: float = 0.5        # raw speed below the MUTED edge this
                                       # long -> mute, even mid-event
...
class GpioLine:
    ...
    def set(self, on: bool, force: bool = False):
        ...
        if self._closed:
            return                     # released: nothing left to drive
        with self._lock:
    ...
    def close(self):
        """Drive LOW, let it land, then release. Safe to call twice: the
        finally-block and the atexit hook both call it."""
        if self._closed:
            return
        try:
            self.set(False, force=True)
...
class VehicleSpeed:
    ...
    def __init__(self, max_kmh=250.0):
        ...
        self.rejected = 0
        self.peak = 0.0
        self.samples = 0                 # every byte/line that arrived
        self._rate_hist = deque()        # (time, samples) over ~2 s

    def note_samples(self, n):
        """Count raw arrivals (a read() can deliver several bytes at once)."""
        now = time.monotonic()
        with self._lock:
            self.samples += n
            self._rate_hist.append((now, self.samples))
            while self._rate_hist and now - self._rate_hist[0][0] > 2.0:
                self._rate_hist.popleft()

    def rate(self, now=None):
        """Readings per second over the last ~2 s (0 if nothing recent)."""
        now = time.monotonic() if now is None else now
        with self._lock:
            h = list(self._rate_hist)
        if len(h) < 2 or now - h[-1][0] > 2.0:
            return 0.0
        span = h[-1][0] - h[0][0]
        return (h[-1][1] - h[0][1]) / span if span > 0 else 0.0
...
class SlabSelector:
    ...
    def __init__(self, cfg: Config, table: SlabTable, speed: VehicleSpeed = None,
        ...
        self._baseline = {}
        self._below_mute_since = None
        first = table.slabs[0].values
        # Only a first slab with EVERY level off gets the mute behaviour. If
        # you give MUTED an alarm, it becomes an ordinary slab again.
        self.mute_capable = (len(table.slabs) > 1 and
                             all(first.get(k) is None for k in SLAB_LEVEL_KEYS))

    @property
    def mute_edge(self):
        return self.table.slabs[1].from_kmh if len(self.table.slabs) > 1 else INF

    @property
    def muted(self):
        return self.armed and self.mute_capable and self.idx == 0
    ...
    def update(self, now, hold=False):
        ...
        if stale:
            ...
            # ---- MUTE: its own fast rule, deliberately NOT held by events --
            # Confirmed on RAW readings for mute_confirm_s, so one corrupt 0
            # byte at 100 km/h cannot silence the system.
            if self.mute_capable:
                if raw < self.mute_edge:
                    if self._below_mute_since is None:
                        self._below_mute_since = now
                    if self.idx != 0 and \
                       now - self._below_mute_since >= c.mute_confirm_s:
                        self.kmh = raw
                        msgs.append(self._switch(0, now))
                        return msgs
                else:
                    self._below_mute_since = None
            ...
            if self.kmh is None or dt <= 0.0:
                ...
                if c.speed_smooth_s <= 0:
                    self.kmh = raw
                else:
                    a = 1.0 - math.exp(-dt / c.speed_smooth_s)
                    self.kmh += a * (raw - self.kmh)
            want = t.index_for(self.kmh)
            # Going FASTER is decided on the RAW reading, with no smoothing
            # delay: it is the safe direction (stricter ladder). Going slower
            # still uses the smoothed speed + hysteresis + dwell below.
            raw_idx = t.index_for(raw)
            if raw_idx > want:
                want = raw_idx
            if self.muted:
                # Leaving MUTED is decided on the raw reading alone, and the
                # smoother restarts from it (no stale lag after a stop).
                want = raw_idx
                self.kmh = raw
    ...
    def vehicle_text(self, now=None):
        ...
        r = self.speed.rate(now)
        every = f", every {1000.0 / r:.0f} ms" if r > 0 else ""
        return f"vehicle {v:.0f} km/h ({r:.1f}/s{every})"
    ...
    def _rx_worker(self):
        ...
        while not self._stop.is_set():
            ...
            if self.rx_format == "byte":
                self.speed_in.note_samples(len(data))
                self.speed_in.push(data[-1])     # newest byte wins
                continue
...
class SafetyPolicy:
    ...
    def force_normal(self, now, why):
        """Drop to NORMAL at once and forget the repetition tally."""
        self._micro.clear()
        self._last_micro = None
        ...
        if self.level != self.NORMAL:
            self._transition(self.NORMAL, now, why)
...
class Alerter:
    ...
    def mute(self, led_mode="face", face=None):
        """
        Buzzer and speed byte off. Called every frame while MUTED.
        The LED is a SENSOR indicator, not a safety one, so by default it
        keeps showing face presence while muted:
          face   lit while a face is visible (same as when driving)
          blink  short flash every 2 s: 'muted, alive'
          off    dark
        """
        if self._alarm_active:
            self._alarm_active = False
            self._say("[ALERT] alarm cleared  (vehicle stopped)")
        self._warned_at_level = False
        self._noface_since = None
        self._noface_active = False
        self._face_seen = False
        ...
        if self.led:
            if led_mode == "face":
                self._update_led(bool(face))
                return
            self._led_off_at = None
            now = time.monotonic()
            want = (now % 2.0) < 0.12 if led_mode == "blink" else False
            force = (now - self._led_forced) >= self.led_reassert_s
            if force:
                self._led_forced = now
            self.led.set(want, force=force)
...
class MicrosleepDetector:
    ...
    def pause(self):
        """
        Forget every event in progress. Called when the vehicle stops: a
        closure that began at 60 km/h must not complete as a 'sleep' after
        the car has been standing for a minute.
        """
        self.eyes_shut = False
        self.shut_start = 0.0
        self.shut_frames = 0
        ...
        self.trend_risk = 0.0
        self.last_t = None
        self.last_blink_t = 0.0        # re-armed on the first frame back
        self.state.status = "MUTED"
        self.state.risk = 0.0
        self.state.trend_risk = 0.0
        self.state.closure_s = 0.0
        self.state.stare_s = 0.0
        self.state.reasons = ["vehicle stopped"]
...
def build_parser():
    ...
    ap.add_argument("--muted-led", choices=("face", "blink", "off"),
                    default="face",
                    help="LED while MUTED: face = still shows face presence "
                         "(runs face detection, no drowsiness logic); "
                         "blink = short flash every 2 s; off = dark")
    ...
    sp.add_argument("--speed-smooth", type=float, default=CFG.speed_smooth_s,
                    help="seconds of smoothing before moving to a SLOWER slab "
                         "(faster slabs use the raw reading, no delay)")
    sp.add_argument("--mute-confirm", type=float, default=CFG.mute_confirm_s,
                    help="seconds the raw speed must stay below the MUTED edge "
                         "before detection pauses")
...
def apply_args_to_config(args):
    ...
    CFG.speed_smooth_s = max(0.0, args.speed_smooth)
    CFG.mute_confirm_s = max(0.0, args.mute_confirm)
...
def serial_selftest(args):
    ...
    if rx:
        ...
        if vs.samples:
            per_s = vs.samples / 6.2
            print(f"  RX rate: about {per_s:.1f} readings/s "
                  f"= one every {1000.0 / per_s:.0f} ms "
                  f"(the link can carry up to {args.serial_baud // 10}/s "
                  f"at {args.serial_baud} baud)")
        else:
            print("  RX rate: NOTHING received -- check AVR TX -> Pi RX "
                  "(pin 10) and common GND")
...
def main(argv=None):
    ...
    try:
        ...
        blocked_noted = False
        was_muted = False
        muted_beat = time.monotonic()

        def infer(frame, t):
            """One MediaPipe pass. Timestamps must strictly increase."""
            nonlocal last_ts_ms
        ...
        def muted_tick(now, face=None):
            """
            Returns True while MUTED -- the caller then skips detection,
            policy and status output for this frame entirely.
            """
            nonlocal was_muted, muted_beat, last_level, last_status
            m_now = selector is not None and selector.muted
            if m_now and not was_muted:
                was_muted = True
                det.pause()
                policy.force_normal(now, "vehicle stopped -- muted")
                last_level = SafetyPolicy.NORMAL
                last_status = "MUTED"
                muted_beat = now
                print(f"[{time.strftime('%H:%M:%S')}] [MUTED] vehicle below "
                      f"{selector.mute_edge:g} km/h -- detection paused, "
                      f"buzzer and speed byte off"
                      + (", LED still shows face presence"
                         if args.muted_led == "face" else
                         f", LED {args.muted_led}"))
            elif not m_now and was_muted:
                was_muted = False
                det.pause()                  # start clean, no carried events
                print(f"[{time.strftime('%H:%M:%S')}] [MUTED] off -- "
                      f"detection resumed ({selector.describe()})")
            if m_now:
                alerter.mute(args.muted_led, face)
                if args.heartbeat and now - muted_beat >= args.heartbeat:
                    muted_beat = now
                    print(f"[{time.strftime('%H:%M:%S')}] MUTED  "
                          f"[{selector.vehicle_text(now)}]")
            return m_now
        ...
        while not stop.is_set():
            ...
            try:
                ...
                # ---- MUTED: no inference, no events, no output -----------
                adapt(now)
                show_speed(now)
                muted_now = selector is not None and selector.muted
                face_now = None
                if muted_now and args.muted_led == "face":
                    # Face presence ONLY, for the LED. The result never
                    # reaches the detector or the policy.
                    face_now = bool(infer(frame, t).face_blendshapes)
                if muted_tick(now, face_now):
                    level = SafetyPolicy.NORMAL
                    last_proc = now
                    frames += 1
                    frame_errors = 0
                ...
                # ---- policy, then actuators (slab chosen above) ----------
                level, why = policy.update(now, ev)
